...
var BA_Active = true;
var Step_CTRL = false;
var Silent_CTRL = true;
var SilentChangedTo1 = true;
var SilentChangedTo0 = false;
...
function DrawMatrix()
{
    ...
    if (SilentChangedTo1)
    {
        ...
        {
            MatrixChangeMS = Date.now();
            matrix.draw(SILENT_M);
            SilentChangedTo1 = false;
            previousSignal = 0;
        }
    }
    if (SilentChangedTo0)
...
var SilentLastTriggering = Date.now();
...
io.sockets.on("connection", function(socket) {  // od oklepaja ( dalje imamo argument funkcije on -> ob 'connection' se prenese argument t.j. funkcija(socket) 
	...
	socket.on("CommandToArduinoMove", function(data) {
        ...
        if (data.CommandNumber == 12 && (Date.now() - SilentLastTriggering) > 3000)
        {
            Silent_CTRL = 1-Silent_CTRL;
            SilentLastTriggering = Date.now();
            if (Silent_CTRL == 1)
                SilentChangedTo1 = 1;
            else
                SilentChangedTo0 = 1;
