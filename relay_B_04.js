...
function handler (req, res) { // handler za "response"; ta handler "handla" le datoteko index.html
    fs.readFile(__dirname + "/relay_B_04.html",
    function (err, data) {
        if (err) {
            res.writeHead(500, {'Content-Type': 'text/plain'});
            return res.end("Napaka pri nalaganju datoteke relay_B_04.html");
        }
    res.writeHead(200);
    res.end(data);
    });
...
var board = new firmata.Board("/dev/ttyACM1",function(){
	...
	board.pinMode(24, board.MODES.INPUT); // LEFT digital pin from encoder    
...
io.sockets.on("connection", function(socket) {  // od oklepaja ( dalje imamo argument funkcije on -> ob 'connection' se prenese argument t.j. funkcija(socket) 
    ...
    var timesArrayMiddle = new Array();
    var timesArrayMiddle = []; // ob povezavi klienta matriko brišemo        
    ...
    var secondMiddleFlag = 0; // zastavica, da vemo, da sta iz MIDDLE enkoderja prišli vsaj dve vrednosti        
    ...
    board.digitalRead(22, function(value) { // LEFT funkcija se aktivira le, kadar se spremeni stanje; sicer bi bilo 1M čitanj na sekundo
        ...
        socket.emit("sporociloKlientu", " 1");
    ...
    board.digitalRead(24, function(value) { // LEFT funkcija se aktivira le, kadar se spremeni stanje; sicer bi bilo 1M čitanj na sekundo
        if (secondMiddleFlag < 1) { // ta del rabimo, da se ne zgodi, da nam ob vklopu, ko kolesa mirujejo digitalRead prebere 1 - kolo sicer miruje (enko vedno prebre) in bi nato narobe preračunali frekvenco 1/0.5=2 V resnici kolo miruje. Prvi preračun lahko naredimo le, ko se pojavi naslednja vrednost
            secondMiddleFlag++;
        }
        else
        {
        timesArrayMiddle.push(Date.now());
        socket.emit("klientBeri1", {"vrednost": value, "cas": timesArrayLeft[timesArrayLeft.length - 1]});
        }
        
        socket.emit("sporociloKlientu", " 2");
    ...
    board.digitalRead(52, function(value) { // RIGHT funkcija se aktivira le, kadar se spremeni stanje; sicer bi bilo 1M čitanj na sekundo
        ...
        socket.emit("sporociloKlientu", " 3");
